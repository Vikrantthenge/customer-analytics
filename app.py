...
if page == "Overview":
    ...
    def generate_pdf_bytes(df):
        ...
        pdf.cell(0, 8, "Customer Segment Summary", ln=True)
        pdf.ln(4)
        # table-like print
        for _, row in df.iterrows():
            line = " | ".join([f"{k}: {round(v,2) if isinstance(v,(float,int)) else v}" for k, v in row.to_dict().items()])
    ...
    # Insights expander (static for now; can be dynamic)
    with st.expander("Insights"):
        st.markdown(
            "- Elite segments often contribute disproportionate revenue. \n"
            "- Consider win-back campaigns for Dormant segments. \n"
            "- Frequency and monetary value are positively correlated for top segments."
        )
